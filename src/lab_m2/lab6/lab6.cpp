...
void Lab6::Init() {
    ...
    camera->SetPositionAndRotation(glm::vec3(0, -1, 4),
                                   glm::quat(glm::vec3(RADIANS(10), 0, 0)));
    camera->Update();

    std::string texturePath =
        PATH_JOIN(window->props.selfDir, RESOURCE_PATH::TEXTURES, "cube");
    std::string shaderPath =
        PATH_JOIN(window->props.selfDir, SOURCE_PATH::M2, "lab6", "shaders");
    ...
    // Create a shader program for rendering cubemap texture
    {
        ...
        shader->AddShader(PATH_JOIN(shaderPath, "CubeMap.VS.glsl"),
                          GL_VERTEX_SHADER);
        shader->AddShader(PATH_JOIN(shaderPath, "CubeMap.FS.glsl"),
                          GL_FRAGMENT_SHADER);
    ...
    // Create a shader program for standard rendering
    {
        ...
        shader->AddShader(PATH_JOIN(shaderPath, "Normal.VS.glsl"),
                          GL_VERTEX_SHADER);
        shader->AddShader(PATH_JOIN(shaderPath, "Normal.FS.glsl"),
                          GL_FRAGMENT_SHADER);
        shader->CreateAndLink();
        shaders[shader->GetName()] = shader;
    }
    ...
    // Create a shader program for creating a CUBEMAP
    {
        ...
        shader->AddShader(PATH_JOIN(shaderPath, "Framebuffer.VS.glsl"),
                          GL_VERTEX_SHADER);
        shader->AddShader(PATH_JOIN(shaderPath, "Framebuffer.FS.glsl"),
                          GL_FRAGMENT_SHADER);
        shader->AddShader(PATH_JOIN(shaderPath, "Framebuffer.GS.glsl"),
                          GL_GEOMETRY_SHADER);
        shader->CreateAndLink();
        shaders[shader->GetName()] = shader;
    }
    ...
    cubeMapTextureID =
        UploadCubeMapTexture(PATH_JOIN(texturePath, "pos_x.png"),
    ...
    TextureManager::LoadTexture(PATH_JOIN(window->props.selfDir,
                                          RESOURCE_PATH::MODELS, "characters",
                                          "archer", "Akai_E_Espiritu.fbm"),
                                "akai_diffuse.png");

    // Create the framebuffer on which the scene is rendered from the
    // perspective of the mesh Texture size must be cubic
...
void Lab6::Update(float deltaTimeSeconds) {
    ...
    if (framebuffer_object) {
...

        {
            ...
            glUniform1i(
                glGetUniformLocation(shader->program, "texture_cubemap"), 1);
        ...
        for (int i = 0; i < 5; i++) {
            ...
            modelMatrix *=
                glm::rotate(glm::mat4(1), angle + i * glm::radians(360.0f) / 5,
                            glm::vec3(0, 1, 0));
            modelMatrix *= glm::translate(glm::mat4(1), glm::vec3(3, -1, 0));
            modelMatrix *= glm::rotate(glm::mat4(1), glm::radians(-90.0f),
                                       glm::vec3(0, 1, 0));
            ...
            glUniformMatrix4fv(
                glGetUniformLocation(shader->GetProgramID(), "viewMatrices"), 6,
                GL_FALSE, glm::value_ptr(cubeView[0]));
            glUniformMatrix4fv(shader->loc_projection_matrix, 1, GL_FALSE,
                               glm::value_ptr(projection));
            ...
            glBindTexture(
                GL_TEXTURE_2D,
                TextureManager::GetTexture("akai_diffuse.png")->GetTextureID());
    ...
    // Draw the reflection on the mesh
    {
        ...
        glUniform3f(loc_camera, cameraPosition.x, cameraPosition.y,
                    cameraPosition.z);
...
unsigned int
Lab6::UploadCubeMapTexture(const std::string &pos_x, const std::string &pos_y,
                           const std::string &pos_z, const std::string &neg_x,
                           const std::string &neg_y, const std::string &neg_z) {
    int width, height, chn;

    unsigned char *data_pos_x =
        stbi_load(pos_x.c_str(), &width, &height, &chn, 0);
    unsigned char *data_pos_y =
        stbi_load(pos_y.c_str(), &width, &height, &chn, 0);
    unsigned char *data_pos_z =
        stbi_load(pos_z.c_str(), &width, &height, &chn, 0);
    unsigned char *data_neg_x =
        stbi_load(neg_x.c_str(), &width, &height, &chn, 0);
    unsigned char *data_neg_y =
        stbi_load(neg_y.c_str(), &width, &height, &chn, 0);
    unsigned char *data_neg_z =
        stbi_load(neg_z.c_str(), &width, &height, &chn, 0);
...
void Lab6::CreateFramebuffer(int width, int height) {
    ...
    if (depth_texture) {
        glFramebufferTexture(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, depth_texture,
                             0);
    }
