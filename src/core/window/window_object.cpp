...
WindowObject::WindowObject(WindowProperties properties) : props(properties) {
...
void WindowObject::PollEvents() const { glfwPollEvents(); }
...
void WindowObject::FullScreen() {
    GLFWmonitor *monitor = glfwGetPrimaryMonitor();
    const GLFWvidmode *videoDisplay = glfwGetVideoMode(monitor);
    window->handle = glfwCreateWindow(videoDisplay->width, videoDisplay->height,
                                      props.name.c_str(), monitor, NULL);
...
void WindowObject::WindowMode() {
    ...
    window->handle = glfwCreateWindow(props.resolution.x, props.resolution.y,
...
bool WindowObject::KeyHold(int keyCode) const { return keyStates[keyCode]; }

bool WindowObject::MouseHold(int button) const {
    return IS_BIT_SET(mouseButtonStates, button);
}

int WindowObject::GetSpecialKeyState() const { return keyMods; }

glm::ivec2 WindowObject::GetCursorPosition() const { return props.cursorPos; }
...
void WindowObject::MouseButtonCallback(int button, int action, int mods) {
    ...
    action ? SET_BIT(mouseButtonStates, button)
           : CLEAR_BIT(mouseButtonStates, button);
}
...
void WindowObject::UpdateObservers() {
    ...
    if (mouseMoveEvent) {
        mouseMoveEvent = false;
        for (auto obs : observers) {
            obs->OnMouseMove(props.cursorPos.x, props.cursorPos.y, mouseDeltaX,
                             mouseDeltaY);
        }
    }
    ...
    if (pressEvent) {
        for (auto obs : observers) {
            obs->OnMouseBtnPress(props.cursorPos.x, props.cursorPos.y,
                                 pressEvent, keyMods);
        }
    }
    ...
    if (releaseEvent) {
        for (auto obs : observers) {
            obs->OnMouseBtnRelease(props.cursorPos.x, props.cursorPos.y,
                                   releaseEvent, keyMods);
        }
    }
    ...
    if (scrollEvent) {
        scrollEvent = false;
        for (auto obs : observers) {
            obs->OnMouseScroll(props.cursorPos.x, props.cursorPos.y,
                               mouseScrollDeltaX, mouseScrollDeltaY);
        }
    }
    ...
    if (registeredKeyEvents) {
        for (int i = 0; i < registeredKeyEvents; i++) {
            for (auto obs : observers)
                keyStates[keyEvents[i]]
                    ? obs->OnKeyPress(keyEvents[i], keyMods)
...
void WindowObject::SetSize(int width, int height) {
    ...
    glfwGetFramebufferSize(window->handle, &frameBufferWidth,
                           &frameBufferHeight);
...
void WindowObject::SwapBuffers() const { glfwSwapBuffers(window->handle); }
