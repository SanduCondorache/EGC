...
void Lab8::Init() {
    ...
    // normal
    {
        ...
        shader->AddShader(PATH_JOIN(window->props.selfDir, SOURCE_PATH::M1,
                                    "lab8", "shaders", "VertexShader.glsl"),
                          GL_VERTEX_SHADER);
        shader->AddShader(PATH_JOIN(window->props.selfDir, SOURCE_PATH::M1,
                                    "lab8", "shaders", "FragmentShader.glsl"),
                          GL_FRAGMENT_SHADER);
...
void Lab8::Update(float deltaTimeSeconds) {
    ...
    }
    {
        ...
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.1f));
        RenderMesh(meshes["sphere1"], shaders["Simple"], modelMatrix);
    }
}
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader,
    ...
    int light_position1 =
        glGetUniformLocation(shader->program, "light_position1");
    glUniform3f(light_position1, lightPosition1.x, lightPosition1.y,
                lightPosition1.z);

    int light_direction1 =
        glGetUniformLocation(shader->program, "light_direction1");
    glUniform3f(light_direction1, lightDirection1.x, lightDirection1.y,
                lightDirection1.z);
