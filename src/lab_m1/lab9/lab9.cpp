...
void Lab9::Init() {
    ...
    // Load textures
    {
        ...
        texture->Load2D(
            PATH_JOIN(sourceTextureDir, "grass_bilboard.png").c_str(),
            GL_REPEAT);
    ...
    // normal
    {
        ...
        shader->AddShader(PATH_JOIN(window->props.selfDir, SOURCE_PATH::M1,
                                    "lab9", "shaders", "VertexShader.glsl"),
                          GL_VERTEX_SHADER);
        shader->AddShader(PATH_JOIN(window->props.selfDir, SOURCE_PATH::M1,
                                    "lab9", "shaders", "FragmentShader.glsl"),
                          GL_FRAGMENT_SHADER);
...
void Lab9::Update(float deltaTimeSeconds) {
    ...
    // Why does this happen? How can you solve it?
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix,
                         mapTextures["earth"], mapTextures["random"]);
    }

    {
        ...
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix,
                         mapTextures["crate"], mapTextures["earth"]);
    }

    {
        ...
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.75f));
        modelMatrix =
            glm::rotate(modelMatrix, RADIANS(75.0f), glm::vec3(1, 1, 0));
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix,
                         mapTextures["random"]);
    }

    {
        ...
        glm::mat3 rot = glm::mat3(viewMatrix);
        glm::mat3 brot = glm::transpose(rot);

        modelMatrix *= glm::mat4(brot);
        RenderSimpleMesh(meshes["square"], shaders["LabShader"], modelMatrix,
                         mapTextures["grass"]);
    }

    {
        ...
        RenderSimpleMesh(meshes["bamboo"], shaders["LabShader"], modelMatrix,
                         mapTextures["bamboo"]);
    }
}

void Lab9::FrameEnd() { DrawCoordinateSystem(); }
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader,
    ...
    if (texture1) {
        ...
        if (string(mesh->GetMeshID()) == "box") {
            glUniform1i(glGetUniformLocation(shader->program, "two"), 0);
        } else if (string(mesh->GetMeshID()) == "sphere") {
            glUniform1i(glGetUniformLocation(shader->program, "two"), 2);
    ...
    if (texture2) {
        ...
        if (string(mesh->GetMeshID()) == "box") {
            glUniform1i(glGetUniformLocation(shader->program, "two"), 1);
        }
    }
...
Texture2D *Lab9::CreateRandomTexture(unsigned int width, unsigned int height) {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                    GL_REPEAT); // modul de wrapping pe orizontala
