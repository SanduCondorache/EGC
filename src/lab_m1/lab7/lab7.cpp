...
void Lab7::Init() {
    ...
    // normal
    {
        ...
        shader->AddShader(PATH_JOIN(window->props.selfDir, SOURCE_PATH::M1,
                                    "lab7", "shaders", "VertexShader.glsl"),
                          GL_VERTEX_SHADER);
        shader->AddShader(PATH_JOIN(window->props.selfDir, SOURCE_PATH::M1,
                                    "lab7", "shaders", "FragmentShader.glsl"),
                          GL_FRAGMENT_SHADER);
...
void Lab7::FrameEnd() { DrawCoordinateSystem(); }

void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader,
    ...
    // materialKd = 0.5;
    // materialKs = 0.5;
    glUniform3fv(shader->GetUniformLocation("light_position"), 1,
                 glm::value_ptr(lightPosition));

    glm::vec3 eyePosition = GetSceneCamera()->m_transform->GetWorldPosition();
    // TODO(student): Set eye position (camera position) uniform
    glUniform3fv(shader->GetUniformLocation("eye_position"), 1,
                 glm::value_ptr(eyePosition));

    // TODO(student): Set material property uniforms (shininess, kd, ks, object
    // color)
    glUniform1i(shader->GetUniformLocation("material_shininess"),
                materialShininess);
    glUniform1f(shader->GetUniformLocation("material_kd"), materialKd);
    glUniform1f(shader->GetUniformLocation("material_ks"), materialKs);
    glUniform3fv(shader->GetUniformLocation("object_color"), 1,
                 glm::value_ptr(color));
