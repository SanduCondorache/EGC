...
gfxc::TextRenderer::TextRenderer(const std::string &selfDir, GLuint width,
                                 GLuint height) {
    // Load and configure shader
    Shader *shader = new Shader("ShaderText");
    shader->AddShader(
        PATH_JOIN(selfDir, RESOURCE_PATH::SHADERS, "Text.VS.glsl"),
        GL_VERTEX_SHADER);
    shader->AddShader(
        PATH_JOIN(selfDir, RESOURCE_PATH::SHADERS, "Text.FS.glsl"),
        GL_FRAGMENT_SHADER);
    ...
    glUniformMatrix4fv(
        ...
        glm::value_ptr(glm::ortho(0.0f, static_cast<GLfloat>(width),
...
void gfxc::TextRenderer::RenderText(std::string text, GLfloat x, GLfloat y,
    ...
    int loc_text_color =
        glGetUniformLocation(this->m_textShader->program, "textColor");
